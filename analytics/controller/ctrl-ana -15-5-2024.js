        ...
        request.query(query,function(err,records){
            ...
            console.log(query);
               ...
               for (var i = 0; i < records['recordsets'][0].length; i++) { 
                ...
                data.push({alarmid: records['recordsets'][0][i]['alarmid'],equipment: records['recordsets'][0][i]['associatedequiptype'], name:records['recordsets'][0][i]['taskname'], city: records['recordsets'][0][i]['cityname'],campus: records['recordsets'][0][i]['campusname'],assigneddate: records['recordsets'][0][i]['taskassigneddate'],priority: records['recordsets'][0][i]['taskpriority'],closeddate: records['recordsets'][0][i]['taskcloseddate'],closeddescription: records['recordsets'][0][i]['taskcloseddesc'],closeddate: records['recordsets'][0][i]['taskcloseddate'],repeatedindays: records['recordsets'][0][i]['taskrepeatdays'],escalationstage: records['recordsets'][0][i]['escalationstage']});
        ...
        request.query(maxsql,function(err,records){
            ...
            console.log(err);
                ...
                //return;
                    ...
                    query2 = "INSERT INTO ["+dbName+"].[ECCAnalytics].[Task] ([taskid],[alarmid],[taskname],[taskpriority],[taskassigneddate],[buildingname],[associatedequiptype],[taskstatus],[escalationstage]) VALUES ('"+taskid.toString()+"',"+req.body.alarmid+",'"+req.body.taskname+"','"+req.body.taskprioirty+"', CURRENT_TIMESTAMP,'"+req.body.buildingname+"','"+req.body.equipmentname+"','1','"+req.body.escalationstage+"');"
                    //console.log(query2)
                    //return 0;
